...
export default {
  validateRegistrationInput: data => {
    ...
    if (!data.user.confirmPassword) {
      errors["confirmPassword"] = T.translate(
        "sign_up.password_confirmation_required"
      );
    }
    if (data.user.password && data.confirmPassword) {
      if (!Validator.equals(data.password, data.confirmPassword)) {
        errors["confirmPassword"] = T.translate(
          "sign_up.password_match_required"
        );
      }
    }
  ...
  validateUserPasswordInput: data => {
    ...
    if (!data.currentPassword) {
      errors["currentPassword"] = T.translate(
        "settings.user.current_password_required"
      );
    }
    ...
    if (data.newPassword && data.confirmNewPassword) {
      if (!Validator.equals(data.newPassword, data.confirmNewPassword)) {
        ...
        errors["confirmNewPassword"] = T.translate(
          "sign_up.password_match_required"
        );
      }
    }
  ...
  validateTaskInput: data => {
    ...
    if (!data.paymentType) {
      errors["paymentType"] = T.translate(
        "projects.tasks.form.payment_type_required"
      );
    }
    if (!data.hours) {
    ...
    if (!data.unitPrice) {
      errors["unitPrice"] = T.translate(
        "projects.tasks.form.unit_price_required"
      );
    }
  ...
  validateInvoiceInput: data => {
    ...
    if (data.currentStep === "step2") {
      if (data.step2.deadline && data.step2.paymentTerm) {
        errors["deadline"] = T.translate(
          "invoices.form.deadline_or_payment_required"
        );
        errors["paymentTerm"] = T.translate(
          "invoices.form.deadline_or_payment_required"
        );
      }
      if (!data.step2.interestInArrears) {
        errors["interestInArrears"] = T.translate(
          "invoices.form.intereset_in_arrears_required"
        );
      }
      if (!data.step2.tax) {
  ...
  validateContactMessageInput: data => {
    ...
    if (!data.messageBody) {
      errors["messageBody"] = T.translate(
        "landing.contacts.message_body_required"
      );
    }
