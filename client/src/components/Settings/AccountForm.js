...
class AccountForm extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      subdomain: this.props.data.getAccount
        ? this.props.data.getAccount.subdomain
        : "",
      logoUrl: this.props.data.getAccount
        ? this.props.data.getAccount.logoUrl
        : "",
      industry: this.props.data.getAccount
        ? this.props.data.getAccount.industry
        : "",
      currencyCode: this.props.data.getAccount
        ? this.props.data.getAccount.currencyCode
        : "USD",
      companyId: this.props.data.getAccount
        ? this.props.data.getAccount.companyId
          ? this.props.data.getAccount.companyId
          : ""
        : "",
      address: {
        street: this.props.data.getAccount
          ? this.props.data.getAccount.street
            ? this.props.data.getAccount.street
            : ""
          : "",
        postalCode: this.props.data.getAccount
          ? this.props.data.getAccount.postalCode
            ? this.props.data.getAccount.postalCode
            : ""
          : "",
        region: this.props.data.getAccount
          ? this.props.data.getAccount.region
            ? this.props.data.getAccount.region
            : ""
          : "",
        country: this.props.data.getAccount
          ? this.props.data.getAccount.country
            ? this.props.data.getAccount.country
            : ""
          : ""
      },
      contact: {
        phoneNumber: this.props.data.getAccount
          ? this.props.data.getAccount.phoneNumber
            ? this.props.data.getAccount.phoneNumber
            : ""
          : "",
        email: this.props.data.getAccount
          ? this.props.data.getAccount.email
            ? this.props.data.getAccount.email
            : ""
          : ""
      },
  ...
  UNSAFE_componentWillReceiveProps = nextProps => {
    if (nextProps.data.getAccount) {
      this.setState({
        ...
        logoUrl: nextProps.data.getAccount.logoUrl
          ? nextProps.data.getAccount.logoUrl
          : "",
        ...
        companyId: nextProps.data.getAccount.companyId
          ? nextProps.data.getAccount.companyId
          : "",
        address: {
          street: nextProps.data.getAccount.street
            ? nextProps.data.getAccount.street
            : "",
          postalCode: nextProps.data.getAccount.postalCode
            ? nextProps.data.getAccount.postalCode
            : "",
          region: nextProps.data.getAccount.region
            ? nextProps.data.getAccount.region
            : "",
          country: nextProps.data.getAccount.country
            ? nextProps.data.getAccount.country
            : ""
        },
        contact: {
          phoneNumber: nextProps.data.getAccount.phoneNumber
            ? nextProps.data.getAccount.phoneNumber
            : "",
          email: nextProps.data.getAccount.email
            ? nextProps.data.getAccount.email
            : ""
        }
      });
    }
  ...
  handleSubmit = e => {
    ...
    if (this.isValid()) {
      ...
      const {
        ...
        logoUrl,
        address: { street, postalCode, region, country }
      } = this.state;
  ...
  uploadToServer = async public_id => {
    ...
    this.props
      .updateAccountMutation({ variables: { subdomain, logoUrl } })
      .then(res => {
        const { success, errors } = res.data.updateAccount;
  ...
  render() {
    ...
    return (
      <Item className="mb-5">
        <Item.Image>
          <Card
            className={classnames("ui form", { loading: isLoadingLogo })}
            style={{ height: "170px" }}
          >
            <Dimmer.Dimmable
          ...
          >
            <Icon name="upload" />
            &nbsp;{T.translate("settings.account.upload")}
          </Button>
        </Item.Image>
        <Item.Content>
          <Header as="h1" className="mt-2 mb-3">
            {T.translate("settings.account.header")}
          </Header>
          ...
          >
            ...
            <Form.Field error={!!errors.industry}>
              ...
              <Select
                ...
                onChange={(e, { value }) =>
                  this.handleChange("industry", value)
                }
            ...
            <Form.Field error={!!errors.industry}>
              ...
              <Select
                placeholder={T.translate(
                  "settings.account.select_currency_code"
                )}
                name="currencyCode"
                value={currencyCode}
                onChange={(e, { value }) =>
                  this.handleChange("currencyCode", value)
                }
            ...
            <fieldset className="custom-fieldset">
              <legend className="custom-legend">
                {T.translate("settings.account.contact.header")}
              </legend>
              <Form.Field>
                <label>
                  {T.translate("settings.account.contact.phone_number")}
                </label>
                <Input
                  placeholder={T.translate(
                    "settings.account.contact.phone_number"
                  )}
                  name="phoneNumber"
            ...
            <fieldset className="custom-fieldset">
              <legend className="custom-legend">
                {T.translate("settings.account.address.header")}
              </legend>
              <Form.Field>
              ...
              <Form.Field>
                <label>
                  {T.translate("settings.account.address.postal_code")}
                </label>
                <Input
                  placeholder={T.translate(
                    "settings.account.address.postal_code"
                  )}
                  name="postalCode"
              ...
              <div>
                ...
                <CountryDropdown
                  defaultOptionLabel={T.translate(
                    "settings.account.address.select_country"
                  )}
              ...
              <div
                className={classnames("field", {
                  error: address.country !== "" && errors["address.region"]
                })}
              >
                <label>{T.translate("settings.account.address.region")}</label>
                <RegionDropdown
                  defaultOptionLabel={T.translate(
                    "settings.account.address.select_region"
                  )}
