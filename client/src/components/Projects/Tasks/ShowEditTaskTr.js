...
export default function ShowEditTaskTr({
  task,
  editTask,
  handleEdit,
  handleCancelEdit,
  ...
  return (
    <tr key={task.id} id={task.id}>
      ...
      <td className="show-task">
        <GetCurrencySymbol />
        {task.total}
      </td>
      <td className="show-task" width="125px">
        <Button.Group fluid size="small" className="show-task">
          <Button basic negative onClick={toggleConfirmationModal}>
            <Icon name="delete" />
          </Button>
          <Button basic positive onClick={handleEdit}>
            <Icon name="edit" />
          </Button>
        </Button.Group>
      </td>
      ...
      <td className="edit-task">
        <Form.Field>
          <Select
            ...
            onChange={(e, { value }) =>
              handleEditTaskChange("paymentType", value, task)
            }
      ...
      <td className="edit-task">
        <Form.Field>
          <Input
            ...
            onChange={(e, { value }) =>
              handleEditTaskChange("hours", value, task)
            }
      ...
      <td className="edit-task">
        <Form.Field>
          <Input
            ...
            onChange={(e, { value }) =>
              handleEditTaskChange("unitPrice", value, task)
            }
            onBlur={handleEditTaskBlur}
      ...
      <td className="edit-task" width="120px">
        <Button.Group size="small" fluid className="edit-task">
          <Button basic onClick={handleCancelEdit}>
            <Icon name="remove" />
          </Button>
          <Button basic positive onClick={handleUpdate}>
