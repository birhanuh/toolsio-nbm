...
class ForgotPasswordRequest extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      email: "",
  ...
  isValid() {
    ...
    const {
      errors,
      isValid
    } = Validation.validateForgotPasswordResetRequestInput({ email });
        ...
        .then(res => {
          ...
          if (success) {
            this.props.addFlashMessage({
              type: "success",
              text: T.translate(
                "log_in.flash.forgot_password_request_email_sent"
              )
            });
    ...
    return (
      <Container text>
        <Header as="h2" image className="turquoise">
          ...
          <Header.Content>
            {T.translate("log_in.request_password_reset")}
          </Header.Content>
        </Header>
        ...
        <Segment>
          ...
          <Form loading={isLoading} onSubmit={this.handleSubmit.bind(this)}>
            ...
            <Button disabled={isLoading} primary fluid>
              {T.translate("log_in.send_request")}
            </Button>
          </Form>
        </Segment>
...
export default connect(
  null,
  { addFlashMessage }
)(graphql(FORGOT_PASSWORD_REQUEST_MUTATION)(ForgotPasswordRequest));
