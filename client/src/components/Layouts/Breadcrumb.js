...
export default function Breadcrumb(props, context) {
  ...
  const breadcrumbSections = filterdTokens.map((token, index) => {
    return index + 1 === filterdTokens.length ? (
      <BreadcrumbElement.Section key={index}>{token}</BreadcrumbElement.Section>
    ) : (
      <BreadcrumbElement.Section key={index} active>
        <Link to={`/${token}`}>{token}</Link>
        <BreadcrumbElement.Divider />
      </BreadcrumbElement.Section>
    );
  });
  ...
  return (
    <Menu fixed="top" className="header-breadcrumb">
      <Menu.Menu position="left">
        <Menu.Item>
          <Header as="h2">
            {T.translate("" + currentPageTitle + "")}
            <Header.Subheader>
              {T.translate("" + currentPageDescription + "")}
            </Header.Subheader>
          </Header>
        </Menu.Item>
      </Menu.Menu>
      <Menu.Menu position="right">
        <Menu.Item>
          <BreadcrumbElement>{breadcrumbSections}</BreadcrumbElement>
        </Menu.Item>
      </Menu.Menu>
    </Menu>
  );
