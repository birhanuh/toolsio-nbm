...
export default function Table({ customers, count, offset, limit }) {
      ...
      <tr>
        <td colSpan="6">
          <Message info className="m-3">
            <Header>
              {T.translate("customers.page.empty_customers_header")}
            </Header>
  ...
  const customersList = (
    <tbody>{customers.map(customer => Tr({ customer: customer }))}</tbody>
  );
      ...
      <thead>
        <tr>
          ...
          <th className="not-sortable">
            {T.translate("customers.page.active_projects_sales")}
          </th>
          <th className="not-sortable">
            {T.translate("customers.page.unpaid_invoices")}
          </th>
