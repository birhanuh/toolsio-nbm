...
import {
  GET_INVOICES_QUERY,
  GET_INVOICE_FORM_QUERY,
  GET_PROJECTS_WITHOUT_INVOICE_QUERY,
  GET_SALES_WITHOUT_INVOICE_QUERY,
...
class Form extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      step1: {
        sale: this.props.data.getInvoice
          ? this.props.data.getInvoice.sale
          : null,
      ...
      step2: {
        deadline: this.props.data.getInvoice
          ? new Date(this.props.data.getInvoice.deadline)
          : new Date(),
      ...
      this.setState({
        ...
        step2: {
          deadline: nextProps.data.getInvoice.deadline
            ? new Date(nextProps.data.getInvoice.deadline)
            : null,
          ...
          description: !nextProps.data.getInvoice.description
            ? ""
            : nextProps.data.getInvoice.description,
  ...
  handleChange = (name, value) => {
    if (this.state.errors[name]) {
      ...
      if (name === "saleId") {
        const sale = this.state.step1.salesWithoutInvoice.find(
        ...
        const project = this.state.step1.projectsWithoutInvoice.find(
          item => item.id === parseInt(value)
        );
      ...
      this.props.getProjectsWithoutInvoiceQuery.fetchMore({
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          this.setState({
            step1: {
              ...this.state.step1,
              projectsWithoutInvoice: fetchMoreResult.getProjectsWithoutInvoice
            }
          });
        }
      });
      ...
      this.props.getSalesWithoutInvoiceQuery.fetchMore({
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          this.setState({
            step1: {
              ...this.state.step1,
              salesWithoutInvoice: fetchMoreResult.getSalesWithoutInvoice
            }
          });
        }
      });
        ...
        this.props
          .updateInvoiceMutation({
            variables: {
              ...
              interestInArrears: parseInt(interestInArrears),
              status,
              description,
              tax: parseFloat(tax),
              projectId: project && parseInt(project.id),
              saleId: sale && parseInt(sale.id),
            ...
            if (success) {
              this.props.addFlashMessage({
                ...
                text: T.translate("invoices.form.flash.success_update", {
                  name: (project && project.name) || (sale && sale.name)
                })
              });
        ...
        this.props
          .createInvoiceMutation({
            variables: {
              ...
              saleId: sale && parseInt(sale.id),
              customerId: parseInt(
                project ? project.customer_id : sale.customer_id
              )
            },
  ...
  render() {
    ...
    const projectsOption =
      projectsWithoutInvoice &&
      map(projectsWithoutInvoice, project => ({
        key: project.id,
        value: project.id,
        text: project.name
      }));
          ...
          <Segment className="invoices">
            {currentStep === "step1" && (
              <SaleProject
                ...
                step1={step1}
                handleSearchChangeProject={this.handleSearchChangeProject}
                handleSearchChangeSale={this.handleSearchChangeSale}
