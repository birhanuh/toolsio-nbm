...
export default function Details({
  ...
  const paymentTermOptions = Array(99)
    .fill()
    .map((key, value) => ({ key: value, value: value, text: value }));
  const paymentTermOptionsWithDefault = [
    {
      key: "default",
      value: "",
      text: T.translate("invoices.form.select_days")
    },
    ...paymentTermOptions
  ];
  return (
    <Form>
      <div className="inline field">
        {id ? (
          <Header>{T.translate("invoices.form.edit_invoice")}</Header>
        ) : (
          <Header as="h1">
      ...
      <fieldset className="custom-fieldset">
        <legend className="custom-legend">
          {T.translate("invoices.form.select_payment_term_or_deadline")}
        </legend>
      ...
      <Form.Field inline>
        ...
        />
        <span className={classnames({ red: !!errors.description })}>
          {errors.status}
        </span>
      </Form.Field>
