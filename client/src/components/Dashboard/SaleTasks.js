...
const SaleTasksCard = () => (
  <Query query={GET_SALE_TASKS_DATA}>
    {({ loading, error, data }) => {
      const countStatus =
        data.getSaleTasksData && data.getSaleTasksData.countStatus;
      const idNameStatus =
        data.getSaleTasksData && data.getSaleTasksData.idNameStatus;
        ...
        countStatus.map(item => {
          if (item.status === "new") {
            newNotification = (
              <Message info>
                <Message.Content>
                  {T.translate("dashboard.sale_tasks.new_sales", {
                    count: item.count
                  })}
                </Message.Content>
              </Message>
          ...
          if (item.status === "delayed") {
            delayedNotification = (
              <Message negative>
                <Message.Content>
                  {T.translate("dashboard.sale_tasks.delayed_sales", {
                    count: item.count
                  })}
                </Message.Content>
              </Message>
            ...
            {newSales &&
              newSales.map(sale => (
                <List.Item
                  key={sale.id}
                  content={
                    <Link to={`/sales/show/${sale.id}`} className="item blue">
                      {sale.name}
                    </Link>
                  }
                />
            ...
            {delayedSales &&
              delayedSales.map(sale => (
                <List.Item
                  key={sale.id}
                  content={
                    <Link to={`/sales/show/${sale.id}`} className="item red">
                      {sale.name}
                    </Link>
                  }
                />
