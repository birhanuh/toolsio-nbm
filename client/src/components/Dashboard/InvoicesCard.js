...
const InvoicesCard = () => (
  <Query query={GET_INVOICES_DATA}>
    {({ loading, error, data }) => {
      ...
      let monthYear = [`Jan/${year}`, `Feb/${year}`, `Mar/${year}`, `Apr/${year}`, `May/${year}`, `Jun/${year}`, `Jul/${year}`, `Aug/${year}`,
      ...
      const countMonth = data.getInvoicesData && data.getInvoicesData.countMonth.map(item => pick(item, ['month', 'count']))
      ...
      let groupByStatus = countStatusMonth && groupBy(countStatusMonth, 'status')

      // Last four month New statuses 
      groupByStatus && groupByStatus['new'] && groupByStatus['new'].map(item => 
        newData[labels.findIndex(label => label === item.month)] = item.count)
      
      // Last four month Paid statuses 
      groupByStatus && groupByStatus['paid'] && groupByStatus['paid'].map(item => 
        paidData[labels.findIndex(label => label === item.month)] = item.count)

      // Last four month Pending statuses 
      groupByStatus && groupByStatus['pending'] && groupByStatus['pending'].map(item => 
        pendingData[labels.findIndex(label => label === item.month)] = item.count)

      // Last four month Overdue statuses 
      groupByStatus && groupByStatus['overdue'] && groupByStatus['overdue'].map(item => 
        overdueData[labels.findIndex(label => label === item.month)] = item.count)
