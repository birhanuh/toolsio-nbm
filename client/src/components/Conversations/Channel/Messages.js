...
const AddUsersToChannelModal = ({
  ...
  >
    ...
    <Modal.Content>
      <UsersForm
        channelId={channelId}
        usersNotInChannel={usersNotInChannel}
        onClose={onClose}
      />
    </Modal.Content>
    <Modal.Actions>
...
class Messages extends Component {
    ...
    this.props.data.subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        return {
          ...prev,
          getChannelMessages: [
            subscriptionData.data.getNewChannelMessage,
  ...
  toggleAddUsersToChannelModal = e => {
    ...
    this.setState(state => ({
      openAddUsersToChannelModal: !state.openAddUsersToChannelModal
    }));
  };

  handleScroll = () => {
    const {
      data: { getChannelMessages, fetchMore },
      receiverId
    } = this.props;
    ...
    if (
      ...
      (getChannelMessages && getChannelMessages.length >= 10)
    ) {
      fetchMore({
        variables: {
  ...
  render() {
    ...
    const {
      getChannelQuery: { getChannel },
      data: { getChannelMessages }
    } = this.props;
    ...
    const emptyMessage = (
      <Message info>
        ...
        <p>
          {T.translate(`conversations.messages.empty_channel_message_message`)}
        </p>
      </Message>
    );
      ...
      <div key="channel-messages" className="messages">
        <Segment clearing vertical className="p-0">
          ...
          <Button
            ...
            size="small"
            floated="right"
            onClick={this.toggleAddUsersToChannelModal.bind(this)}
          >
        ...
        >
          <Comment.Group>
            {getChannelMessages && getChannelMessages.length === 0
              ? emptyMessage
              : messagesList}
          </Comment.Group>
        </div>
