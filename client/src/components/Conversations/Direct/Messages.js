...
const ShowInModal = ({ trigger, src, mimetype }) => {
  ...
  return (
    <Modal trigger={trigger} className="messages">
      {content}
    </Modal>
  );
};
  ...
  if (uploadPath) {
    if (mimetype.startsWith("image/")) {
      ...
      return (
        <ShowInModal
          trigger={
            <div className="ui small message audio">
              <audio controls>
      ...
      return (
        <ShowInModal
          trigger={
            <div className="ui small message video">
              <video controls>
                <source src={uploadPath} type={mimetype} />
              </video>
            </div>
      ...
      return (
        <div className="ui small message pre">
          ...
          <div className="buttons">
            ...
            <a
              ...
              className="ui icon basic small button"
            >
      ...
      // For all rest file types (E.g. text, pdf, doc...()
      return (
        <div className="ui small message pre">
          <pre>{uploadPath}</pre>
          <div className="buttons">
...
class Messages extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.props
      .markDirectMessagesAsReadMutation({
        variables: { senderId: parseInt(this.props.receiverId) },
        update: store => {
          const data = store.readQuery({
            query: GET_UNREAD_DIRECT_MESSAGES_COUNT_SENDER_QUERY
          });

          const updatedUnreadDirectMessagesCountSender = data.getUnreadDirectMessagesCountSender.unreadDirectMessagesCountSender.map(
            item => {
  ...
  UNSAFE_componentWillReceiveProps({ receiverId }) {
    ...
    this.props.data.subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        return {
          ...prev,
          getDirectMessages: [
            subscriptionData.data.getNewDirectMessage,
  ...
  handleScroll = () => {
    const {
      data: { getDirectMessages, fetchMore },
      receiverId
    } = this.props;
    ...
    if (
      ...
      (getDirectMessages && getDirectMessages.length >= 10)
    ) {
      fetchMore({
        variables: {
          receiverId: parseInt(receiverId),
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          return Object.assign({}, prev, {
            getDirectMessages: [
              ...prev.getDirectMessages,
              ...fetchMoreResult.getDirectMessages
            ]
          });
        }
      });
  ...
  render() {
    ...
    const emptyMessage = (
      <Message info>
        ...
        <p>
          {T.translate(`conversations.messages.empty_direct_message_message`)}
        </p>
      </Message>
    );
    ...
    const messagesList =
      getDirectMessages &&
      getDirectMessages.map(message => (
        <Comment key={message.id}>
          <div className="avatar">
            {message.user.avatarUrl ? (
              <CloudinaryImage
                cloudName="toolsio"
                publicId={message.user.avatarUrl}
                gravity="face"
                width="50"
                height="50"
                crop="thumb"
    ...
    return (
      <div className="messages">
        <Segment vertical clearing>
          <Header as="h3" dividing className="capitalize">
        ...
        */}
        ...
        >
          <Comment.Group>
            {getDirectMessages && getDirectMessages.length === 0
              ? emptyMessage
              : messagesList}
          </Comment.Group>
        </div>
